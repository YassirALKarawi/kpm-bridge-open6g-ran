...
\begin{abstract}
Open Radio Access Network (O-RAN) implementations can emit syntactically valid E2 Service Model for Key Performance Measurement (E2SM-KPM) reports that denote different statistical objects for the same artificial-intelligence (AI) control application, or xApp. Units, entity scope, observation windows, clocks, counter resets, missingness, and hidden filtering may differ silently. KPM-Bridge separates E2 interoperability from semantic and statistical portability by compiling typed canonical contracts, aligning event-time observations, learning an anchor-assisted residual map, and attaching a 48-byte evidence certificate. Split-conformal sets cover both the canonical vector and a registered xApp functional. On \DatasetRows\ public ColO-RAN samples with experiment-level separation, seven baselines, ablations, and controlled profiles, KPM-Bridge attains \StableNrmse\ normalized root-mean-square error and \StableAgreement\ canonical-decision agreement across three stationary profiles; the observed reductions relative to the strongest respective baselines are \NrmseGainBest\ and \AgreementGainBest. At 5\% nominal miscoverage, joint coverage is \StableCoverage; admitted actions have \StableSelectiveError\ canonical-action disagreement at \StableAcceptance\ acceptance. Under severe drift, the monitor detects \DriftDetection\ of shifted traces with \DriftDelay\ median delay. Fail-closed invalidation reduces post-shift disagreement exposure from \NoDriftExposure\ to \DriftExposure\ by reducing acceptance to 5.99\%; conditional disagreement nevertheless remains \DriftSelectiveError. The profiles are controlled stressors, not commercial-vendor measurements.
\end{abstract}
...
Figs.~\ref{fig:stationary-nrmse} and \ref{fig:stationary-agreement}, together with Table~\ref{tab:main}, summarize P1--P3. KPM-Bridge obtains mean NRMSE \StableNrmse, an observed \NrmseGainBest\ reduction relative to temporal ridge, the strongest reconstruction baseline. It reaches \StableAgreement\ decision agreement, \AgreementGainBest\ above the same strongest decision baseline. Its mean label regret is 0.0977, compared with 0.1033 for CORAL and 0.1019 for temporal ridge. The whole-trace intervals below overlap, so these differences are descriptive rather than claims of statistical significance.
...
\emph{Reproducibility and external validity:}
One command regenerates the results from pinned public paths, seeds, manifests, tables, and vector figures. The versioned compiler, profiles, baselines, tests, and claim audit are available at \url{https://github.com/YassirALKarawi/kpm-bridge-open6g-ran}; upstream GPL-3.0 traces are not redistributed. External validation should combine xDevSM and KPM-Bridge on OAI, srsRAN, and vendor E2 traces while varying granularity and schedulers. Conditional or block-aware calibration, monotone maps, and cost-aware anchor selection also remain open.

\section{Conclusion}
KPM-Bridge separates E2/API interoperability from the semantic and statistical evidence required by a portable AI xApp. Typed contracts prevent quantity and scope substitution; event-time mapping repairs declared counter and temporal differences; conformal sets expose uncertainty; and drift monitoring invalidates stale calibration. On independent ColO-RAN repetitions, the method yields lower observed stationary reconstruction error and higher canonical-decision agreement than the evaluated portable baselines while making abstention explicit. Under severe drift, fail-closed gating reduces joint disagreement exposure by \DriftExposureReduction, but only by accepting 5.99\% of post-shift samples; conditional disagreement, detection delay, and false alarms remain material limitations. Portable xApps should receive both KPM values and verifiable evidence of their meaning, age, support, and calibration regime.
