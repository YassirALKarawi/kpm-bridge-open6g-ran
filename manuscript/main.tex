...
\begin{abstract}
Open Radio Access Network (O-RAN) implementations can emit syntactically valid E2 Service Model for Key Performance Measurement (E2SM-KPM) reports that denote different statistical objects for the same artificial-intelligence (AI) control application, or xApp. Units, entity scope, observation windows, clocks, counter resets, missingness, and hidden filtering may differ silently. KPM-Bridge separates E2 interoperability from semantic and statistical portability by compiling typed canonical contracts, aligning event-time observations, learning an anchor-assisted residual map, and attaching a 48-byte evidence certificate; split-conformal sets cover both the canonical vector and a registered xApp functional, so a fixed xApp can admit, defer, or abstain. On \DatasetRows\ public ColO-RAN samples with experiment-level separation, seven baselines, ablations, and controlled profiles, KPM-Bridge matches the strongest portable baselines on stationary accuracy---\StableNrmse\ normalized root-mean-square error and \StableAgreement\ canonical-decision agreement, with observed margins (\NrmseGainBest, \AgreementGainBest) well within bootstrap interval widths---while adding evidence no baseline emits: admitted actions show \StableSelectiveError\ conditional disagreement at \StableAcceptance\ acceptance; joint coverage is \StableCoverage. Under severe drift, the monitor detects \DriftDetection\ of shifted traces with \DriftDelay\ median delay; fail-closed invalidation cuts post-shift disagreement exposure from \NoDriftExposure\ to \DriftExposure\ by reducing acceptance to 5.99\%, and conditional disagreement remains \DriftSelectiveError. The profiles are controlled stressors, not commercial-vendor measurements.
\end{abstract}
...
Figs.~\ref{fig:stationary-nrmse} and \ref{fig:stationary-agreement}, together with Table~\ref{tab:main}, summarize P1--P3. KPM-Bridge obtains mean NRMSE \StableNrmse, an observed \NrmseGainBest\ reduction relative to temporal ridge, the strongest reconstruction baseline. It reaches \StableAgreement\ decision agreement, \AgreementGainBest\ above the same strongest decision baseline. Its mean label regret is 0.0977, compared with 0.1033 for CORAL and 0.1019 for temporal ridge. These margins are small relative to the whole-trace bootstrap interval widths reported below, so we read the stationary comparison as accuracy parity with the strongest baselines rather than as a significance claim. The operative difference lies in what accompanies the numbers: at matched accuracy, only KPM-Bridge emits per-sample evidence of meaning, age, support, and calibration state, and the selective results below show that this evidence, not raw accuracy, is what changes the quality of admitted decisions.
...
The Python reference ran on an AMD EPYC 9V74 central processing unit (CPU) with one OpenMP/BLAS thread. KPM-Bridge fits a profile in \BridgeFitRange, takes \BridgeLatency\ per sample in amortized batches, and occupies \BridgeModelSize\ on average; temporal ridge takes \TemporalLatency\ and 13.4~KiB. Fig.~\ref{fig:runtime} and Table~\ref{tab:complexity} distinguish measured batch cost and storage from asymptotic scaling. They exclude E2 decoding, transport, scheduling, queueing, and actuation and therefore are not worst-case deadline claims.
...
The certificate binds a mapping, model, calibration epoch, and measured operating evidence. Its finite-sample guarantee requires the stated exchangeability assumption and is neither conditional coverage for every subgroup nor proof that an xApp is beneficial, fair, secure, or RAN-stable. It certifies telemetry fitness, not system trustworthiness.

\emph{Information loss and anchors:}
Cell aggregation cannot identify UEs without side information, long windows suppress transients, and consecutive losses enlarge uncertainty; the bridge therefore exposes support and abstains. A cybersecurity-driven quantum digital twin similarly maps O-RAN observations into detection registers \cite{alkarawi2026cqdt}; KPM-Bridge can guard input compatibility, not validate its policy. The 10\% anchor budget requires a trusted reference, digital twin, temporary dual reporting, or validated probe. Performance remains useful at 5\%, but anchor cost and integrity are deployment constraints.
...
Operator-governed contracts bind quantity/scope namespaces, E2SM revision, measurement identifier, labels, canonical ontology, and transform hash. Units may convert, UE-to-cell scope is breaking, and window or clock changes require recalibration. The operator, not the xApp, approves sources against model-card requirements for features, age, support, and functional score.
...
The anchor-residual detector cannot observe drift without reference evidence. Under P4, detection is imperfect, \DriftFalseAlarm\ of traces trigger early, and post-shift conditional disagreement remains \DriftSelectiveError; its main benefit is the \DriftExposureReduction\ exposure reduction achieved through abstention. Lower thresholds trade delay for availability, while per-stream-maximum calibration is empirical rather than an anytime-valid false-alarm theorem. Alarms therefore need cause codes and operator diagnostics. An adversary corrupting reports and anchors can evade residual monitoring, whereas induced missingness can force denial of service. Authenticated anchors, access or execution controls, rate limits, redundant evidence, and emergency policies remain platform responsibilities.
