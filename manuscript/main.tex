...
An event is \emph{admitted}, $g_t=1$, only when it clears the support and age budgets, the calibration epoch is drift-valid, and its functional interval avoids the threshold, $g_t=\ind\{s_t\ge s_{\min}\}\ind\{a_t\le a_{\max}\}\ind\{\text{epoch valid}\}\ind\{\theta\notin I_t^\ell\}$; then $g_t=1$ emits $a(\hat z_t)$ and $g_t=0$ abstains with a reason code. Over $T$ events with $\cA=\{t:g_t=1\}$, the acceptance rate, selective (admission-conditional) error, and joint disagreement exposure are
\begin{equation}
\begin{aligned}
\mathrm{acc}=\frac{1}{T}\sum_{t} g_t,\qquad
\mathrm{serr}=\frac{1}{|\cA|}\sum_{t\in\cA}\ind\{a(\hat z_t)\ne a(z_t)\},\\
